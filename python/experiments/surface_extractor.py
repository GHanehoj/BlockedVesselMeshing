...
path = "/media/data/data/meshes/attic/liver/liver_4"
...
def view_cull(tet, view, proj, scale):
    ...
    p_cam = (proj@view@p_hom.T).T
    p_tmp = (p_cam / p_cam[:,3][:,None])[:,:3]
    p_clip = p_tmp/scale

    cull_mask = np.logical_or.reduce((p_clip[:,0] < -1, p_clip[:,0] > 1, p_clip[:,1] < -1, p_clip[:,1] > 1, p_clip[:,2] < 0, p_clip[:,2] > 1))
...
# proj = perspective_fov(np.pi/4, 1.5, 0.005, 20.0)
