...
# tet = load_tet("/media/data/data/meshes/full_4.mesh")

# tet_sample = np.random.choice(np.arange(len(tet.tets)), 1000000)
# r_outs = np.array([compute_circumscribed_sphere(*tet)[1] for tet in tqdm(tet.nodes[tet.tets[tet_sample]])])
# r_ins = np.array([compute_inscribed_sphere(*tet)[1] for tet in tqdm(tet.nodes[tet.tets[tet_sample]])])
...
def plot(data, x_label):
    ...
    sns.histplot(data, ax=ax, stat="density", bins=1000)
